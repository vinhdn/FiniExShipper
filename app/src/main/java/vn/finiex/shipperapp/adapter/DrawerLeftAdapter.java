    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        if (holder.holderID == 1) {
            ...
            if (this.userInfo != null) {
                ...
                try {
                    holder.payment.setText(format.format(userInfo.getAPayment()));
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
                try {

                    holder.outomoney.setText(format.format(userInfo.getAOutoMoney()));
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
                try {

                    holder.money.setText(format.format(userInfo.getPMoney()));
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
                try {

                    holder.ship.setText(format.format(userInfo.getPShip()));
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
                if (avatar != null)
